...
namespace I2.Loc
{
	public partial class Localize
	{
		...
		{
			string second = (mTarget_GUIText.font!=null ? mTarget_GUIText.font.name : string.Empty);
			return SetFinalTerms (mTarget_GUIText.text, 	second, 	out PrimaryTerm, out SecondaryTerm);
		}
		...
		public bool SetFinalTerms_TextMesh(string Main, string Secondary, out string PrimaryTerm, out string SecondaryTerm)
		{
			...
			return SetFinalTerms (mTarget_TextMesh.text, 	second, out PrimaryTerm, out SecondaryTerm);
		}
		...
		void DoLocalize_AudioSource( string MainTranslation, string SecondaryTranslation )
		{
			...
			AudioClip NewClip = (FindTranslatedObject (MainTranslation) as AudioClip);
			if (OldClip == NewClip)
				return;
		...
		void DoLocalize_GUITexture( string MainTranslation, string SecondaryTranslation )
		{
			...
			mTarget_GUITexture.texture = (FindTranslatedObject(MainTranslation) as Texture);
		...
		void DoLocalize_Child( string MainTranslation, string SecondaryTranslation )
		{
			...
			GameObject NewPrefab = (FindTranslatedObject(MainTranslation) as GameObject);
			if (NewPrefab)
			{
