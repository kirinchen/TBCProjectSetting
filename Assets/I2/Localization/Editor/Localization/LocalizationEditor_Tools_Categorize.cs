...
namespace I2.Loc
{
	public partial class LocalizationEditor
	{
		...
		void OnGUI_CategorizedTerm( string Term )
		{
			...
			LocalizationEditorDB.DeserializeFullTerm(Term, out sKey, out sCategory);
		...
		void OnGUI_NewOrExistingCategory()
		{
			//--[ Create Category ]------------------------
			GUILayout.BeginHorizontal();
				mNewCategory = GUILayout.TextField(mNewCategory, EditorStyles.toolbarTextField, GUILayout.ExpandWidth(true)).Trim ("\\/".ToCharArray());
		...
		void AssignCategoryToSelectedTerms()
		{
			...
			if (mNewCategory==LocalizationEditorDB.EmptyCategory)
			...
			for (int i=mSelectedKeys.Count-1; i>=0; --i)
			{
				...
				LocalizationEditorDB.DeserializeFullTerm( OldTerm, out sKey, out sCategory );
				...
				if (!LocalizationEditorDB.mLanguageSource.ContainsTerm(sKey))
				{
					TermData termData = LocalizationEditorDB.mLanguageSource.GetTermData( OldTerm );
					termData.Term = sKey;
				}
			}
			...
			if (mNewCategory == string.Empty) 
				mNewCategory = LocalizationEditorDB.EmptyCategory;
		...
		public static void ReplaceTermsInCurrentScene()
		{
			...
			for (int i=0, imax=Locals.Length; i<imax; ++i)
			{
				...
				string sMainTerm, sSecondaryTerm;
				if (!localize.GetFinalTerms( out sMainTerm, out sSecondaryTerm ))
					continue;

				string NewTerm;
				if (TermReplacements.TryGetValue(sMainTerm, out NewTerm))
					localize.mTerm = NewTerm;

				if (TermReplacements.TryGetValue(sSecondaryTerm, out NewTerm))
